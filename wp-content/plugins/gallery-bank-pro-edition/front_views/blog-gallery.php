...
<style>
	...
	#blog_gallery<?php echo $unique_id;?> {
		width: <?php echo 66 + ($thumbnails_border_size * 2);?>%;
	}
</style>
		...
		switch ($lightbox_type)
		{
			...
			case "fancy_box":
				if ($pics[$flag]->url == "" || $pics[$flag]->url == "undefined" || $pics[$flag]->url == "http://")
				{
					...
					{
						?>
						<a rel="fancybox-button" data-fancybox-group="button" class="<?php echo str_replace(",", " ", $class); ?> fancybox<?php echo $unique_id;?> fancybox-media<?php echo $unique_id;?>" href="<?php echo stripcslashes($pics[$flag]->pic_name); ?>" data-title="<?php echo esc_html($image_title . $image_description); ?>" id="ux_img_div_<?php echo $unique_id; ?>">
						<?php
					}
					else
					{
						?>
						<a rel="fancybox-button" data-fancybox-group="button" class="<?php echo str_replace(",", " ", $class); ?> fancybox<?php echo $unique_id;?> fancybox-buttons<?php echo $unique_id;?>" href="<?php echo stripcslashes(GALLERY_BK_THUMB_URL . $pics[$flag]->thumbnail_url); ?>" data-title="<?php echo esc_html($image_title . $image_description); ?>" id="ux_img_div_<?php echo $unique_id; ?>">
						<?php
					}
				}
				else
			...
			?><div class="blog_gallery margin_thumbs" id="blog_holder<?php echo $unique_id; ?>">
				...
				if ($pics[$flag]->video == 1){
				?>
					<img class="dynamic_css" alt="<?php echo esc_html($image_title . $image_description); ?>" imageid="<?php echo $pics[$flag]->pic_id; ?>" id="ux_img_<?php echo $unique_id; ?>" type="video" src="<?php echo stripcslashes($pics[$flag]->thumbnail_url); ?>" style="width:100%;"/>
				<?php
				} else {
				?>
					<img class="dynamic_css" alt="<?php echo esc_html($image_title . $image_description); ?>" imageid="<?php echo $pics[$flag]->pic_id; ?>" id="ux_img_<?php echo $unique_id; ?>" type="image" src="<?php echo stripcslashes(GALLERY_BK_THUMB_URL . $pics[$flag]->thumbnail_url); ?>" style="width:100%;"/>
				<?php
				}
				switch($effect[0])
				{
