		...
		for ($flag = 0; $flag < count($pics); $flag++)
		{
			...
			switch ($lightbox_type) {
				...
				case "fancy_box":
					if ($pics[$flag]->video == 1) {
						?>
						<a rel="fancybox-button" data-fancybox-group="button" class="imgLiquidFill fancybox<?php echo $unique_id;?> fancybox-media<?php echo $unique_id;?>" href="<?php echo stripcslashes($pics[$flag]->pic_name); ?>" data-title="<?php echo $image_title . $image_description; ?>" id="ux_img_div_<?php echo $unique_id; ?>">
					<?php
					} else {
						?>
						<a rel="fancybox-button" data-fancybox-group="button" class="imgLiquidFill fancybox<?php echo $unique_id;?> fancybox-buttons<?php echo $unique_id;?>" href="<?php echo stripcslashes(GALLERY_BK_THUMB_URL . $pics[$flag]->thumbnail_url); ?>" data-title="<?php echo $image_title . $image_description; ?>" id="ux_img_div_<?php echo $unique_id; ?>">
					<?php
					}
					break;
				case "lightbox2":
