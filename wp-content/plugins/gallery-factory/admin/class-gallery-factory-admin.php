...
if ( ! class_exists( "VLS_Gallery_Factory_Admin" ) ) {
	class VLS_Gallery_Factory_Admin {
		...
		public function init() {
			...
			register_setting( 'vls-gallery-factory', 'vls_gf_lightbox' );
			register_setting( 'vls-gallery-factory', 'vls_gf_display_image_info_on_hover' );
			register_setting( 'vls-gallery-factory', 'vls_gf_pagination_type' );
			register_setting( 'vls-gallery-factory', 'vls_gf_pagination_page_size' );
			register_setting( 'vls-gallery-factory', 'vls_gf_pagination_style' );
			register_setting( 'vls-gallery-factory', 'vls_gf_use_lazy_loading' );
			...
			add_action( "update_option_vls_gf_lightbox", array( $this, 'clear_all_albums_view_cache' ) );
			add_action( "update_option_vls_gf_display_image_info_on_hover", array(
				$this,
				'clear_all_albums_view_cache'
			) );
			add_action( "update_option_vls_gf_pagination_type", array( $this, 'clear_all_albums_view_cache' ) );
			add_action( "update_option_vls_gf_pagination_page_size", array( $this, 'clear_all_albums_view_cache' ) );
			add_action( "update_option_vls_gf_pagination_style", array( $this, 'clear_all_albums_view_cache' ) );
			add_action( "update_option_vls_gf_use_lazy_loading", array( $this, 'clear_all_albums_view_cache' ) );
		...
		public function load_scripts() {
			...
			if ( 'toplevel_page_vls_gf_gallery_manager' == $screen->id ) {
				...
				wp_register_script(
					'vls-gf-jquery-lazyload',
					VLS_GF_PLUGIN_URL . 'lib/jquery-lazyload/jquery.lazyload' . (WP_DEBUG ? '' : '.min') . '.js',
					array('jquery'),
					VLS_GF_VERSION, true
				);
			...
			} else if ( 'tools_page_vls_gf_tools' == $screen->id ) {

				wp_register_script(
					'vls-gf-tools-page',
					VLS_GF_PLUGIN_URL . 'admin/js/tools-page' . ( WP_DEBUG ? '' : '.min' ) . '.js',
					array(
						'jquery'
					),
					VLS_GF_VERSION, true );
				...
				wp_localize_script( 'vls-gf-tools-page', 'vlsGfScriptData', $data );

				wp_enqueue_script( 'vls-gf-tools-page' );
		...
		public function integrateWithVC() {
			...
			vc_map( array(
					"name" => __("Gallery Factory Album", VLS_GF_TEXTDOMAIN),
					"description" => __("Include a Gallery Factory album", VLS_GF_TEXTDOMAIN),
					"base" => "vls_gf_album",
					"class" => "",
					"controls" => "full",
					"icon" => 'vls-gf-icon',
					...
					"params" => array(
							array(
									...
									"holder" => "div",
									"class" => "",
									"heading" => __("Album ID", VLS_GF_TEXTDOMAIN),
									"param_name" => "id",
									"value" => "",
									"description" => __("Enter Gallery Factory album id", VLS_GF_TEXTDOMAIN)
							)
					)
			) );
		}
