	...
	<div class="vls-gf-container">
		...
		<form>
			...
			<?php if ($link_id > 0) { ?>
				...
				<label class="vls-gf-form-element">
					<span><?php _e('Click action', VLS_GF_TEXTDOMAIN); ?></span>
					<select name="click_action">
						<option
							value="lightbox" <?php selected(true, 'lightbox' == $link->click_action); ?>><?php _e('Open in lightbox', VLS_GF_TEXTDOMAIN); ?></option>
						<option
							value="redirect" <?php selected(true, 'redirect' == $link->click_action); ?>><?php _e('Redirect to URL', VLS_GF_TEXTDOMAIN); ?></option>
						<option
							value="page" <?php selected(true, 'page' == $link->click_action); ?>><?php _e('Open dedicated image page (experimental, please refer to the user manual)', VLS_GF_TEXTDOMAIN); ?></option>
						<option
							value="none" <?php selected(true, 'none' == $link->click_action); ?>><?php _e('No action', VLS_GF_TEXTDOMAIN); ?></option>
					</select>
				</label>

				<label class="vls-gf-form-element">
					<span><?php _e('Link URL', VLS_GF_TEXTDOMAIN); ?></span>
					<input type="text" name="link_url" value="<?php echo $link->link_url; ?>"/>
				</label>

				<label class="vls-gf-form-element">
					<span><?php _e( 'Open link in', VLS_GF_TEXTDOMAIN ); ?></span>
					<select name="link_target">
						<option
							value="_self" <?php selected( true, '_self' == $link->link_target ); ?>><?php _e( 'Same window', VLS_GF_TEXTDOMAIN ); ?></option>
						<option
							value="_blank" <?php selected( true, '_blank' == $link->link_target ); ?>><?php _e( 'New window', VLS_GF_TEXTDOMAIN ); ?></option>
					</select>
				</label>
