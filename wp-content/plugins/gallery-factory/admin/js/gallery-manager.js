...
VLS_GF.GalleryManager = (function ($) {
    ...
    function showNameDialog(action, actionFunction) {
        ...
        //cancel button
        var $cancelButton = $('<a class="button" href="#">' + vlsGfGalleryAdminData.l10n.btnCancel + '</a>');
        $cancelButton.on('click.vls-gf', closePopup);
        $buttonsPanel.append($cancelButton);
...
VLS_GF.NavigationPanelModule = (function ($) {
    ...
    function init() {
        ...
        $('#vls-gf-btn-add-new-folder').on('click.vls-gf touchend.vls-gf', galleryTreeAddFolder);
        $('#vls-gf-btn-add-new-album').on('click.vls-gf touchend.vls-gf', galleryTreeAddAlbum);
    ...
    function initGalleryTree() {
        ...
        albumTree.parent().find('li.vls-gf-unsorted, li.vls-gf-album').droppable({
    ...
    function updateGalleryTreeLayout() {
        ...
        for (var i = 0; i < items.length; ++i) {
            ...
            if (level > hideLevel) {
                ...
                if ($item.hasClass('vls-gf-folder') && !$item.hasClass('opened')) {
                ...
                if (!$item.hasClass('ui-draggable-dragging')) {
                    ...
                    if (position == albumTreeDraggingState.ghostPosition - 1) {
                        prevItemIsFolder = $item.hasClass('vls-gf-folder');
                        prevItemLevel = level;
        ...
        if (albumTreeDraggingState.$draggedItem) {
            ...
            if (nextItemLevel <= prevItemLevel) {
                if (prevItemIsFolder) {
                    ...
                    prevItemLevel = prevItemLevel + 1; //if dragging under the folder, allow one level up to drop inside folder

                    //setting the limit for tree depth
                    if (prevItemLevel > 7) {
                        prevItemLevel = 7;
                    }
    ...
    function galleryItemDrag(event, ui) {
        ...
        if (hoveredItem && hoveredItem.hasClass('vls-gf-folder') && !hoveredItem.hasClass('opened')) {
        ...
        for (var i = 0; i <= items.length; ++i) { // used "<=" to allow insertion at last position
            ...
            if (!isPlaced && (
                ...
                for (var j = 0; j < draggingChildItems.length; ++j) {
                    ...
                    newLevel = newLevel > 7 ? 7 : newLevel;
    ...
    function galleryTreeItemClick() {
        ...
        if (!$this.hasClass('ui-draggable-dragging') && !$this.hasClass('ui-draggable-placing')) {
            if ($this.hasClass('vls-gf-folder')) {
                toggleGalleryFolder($this);
            } else if ($this.hasClass('vls-gf-album')) {
                if (!editModeActive && $this.data('vlsGfId') > 0) {
     ...
     * Adds new folder
     */
    function galleryTreeAddFolder(e) {
        galleryTreeAddItem('folder');
        return false;
    }
     ...
     * Adds new album
     */
    function galleryTreeAddAlbum(e) {
        galleryTreeAddItem('album');
        return false;
    }
     ...
     * Adds gallery item
     */
    function galleryTreeAddItem(itemType) {
        ...
        VLS_GF.GalleryManager.showNameDialog(action, function () {
            ...
            if (!itemName || itemName == '') {
                ...
                var $newItem = $('<li class="vls-gf-' + itemType + ' level-1"><a href="#"><span class="vls-gf-icon"></span><span class="vls-gf-label">' + itemName + '</span></a><span class="btn btn-delete"></span><span class="btn btn-rename"></span></li>');
    ...
    function imageToAlbumDrop(e, ui) {
        ...
        var targetAlbum;
        if ($(e.target).hasClass('vls-gf-unsorted')) {
            targetAlbum = 0;
        } else {
...
VLS_GF.AlbumOverviewPanelModule = (function ($) {
    ...
    function initContent(data) {
        ...
        if ($().vlsGfLazyload) {
            imagePanel.find('img[data-original]').vlsGfLazyload({
                threshold: 300,
                effect: 'fadeIn'
            });
        }
...
VLS_GF.AlbumLayoutPanelModule = (function ($) {
    ...
    function initContent(data) {
        ...
        }).append('<span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span><span class="vls-gf-resize-helper"></span>');
